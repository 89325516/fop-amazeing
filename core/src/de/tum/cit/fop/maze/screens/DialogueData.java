...
/**
 * 遊戲對話數據管理類 (Game Dialogue Data Manager)
 * 存儲所有關卡的對話內容和對應的背景圖片路徑
 */
public class DialogueData {

    // 說話者類型枚舉
    public enum Speaker {
        SYSTEM("System", new Color(0.2f, 1f, 0.4f, 1f)),           // 綠色科技風
        DOCTOR("Doctor", new Color(0.7f, 0.85f, 1f, 1f)),          // 淺藍色溫暖風
        ALIEN_SYSTEM("Alien System", new Color(1f, 0.42f, 0.42f, 1f)); // 紅色警告
    ...
    /**
     * 根據關卡號獲取對話數據
     * @param levelNumber 關卡號 (1-20)
     * @return 對應的對話數據，如果關卡號無效則返回 null
     */
    ...
    /**
     * 從地圖路徑提取關卡號
     * @param mapPath 地圖文件路徑，例如 "maps/level-5.properties"
     * @return 關卡號，如果無法解析則返回 -1
     */
    public static int extractLevelNumber(String mapPath) {
        if (mapPath == null) return -1;
        ...
        try {
            ...
            if (levelIndex != -1) {
                ...
                if (endIndex == -1) endIndex = levelPart.length();
