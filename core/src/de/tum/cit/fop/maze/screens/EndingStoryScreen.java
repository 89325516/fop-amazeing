 ...
 * Returns to the main menu after the dialogue ends.
 */
public class EndingStoryScreen implements Screen {
    ...
    private void createDialogBoxTextures() {
        // Create gradient dialog box background - use deeper tones for an ending
        // atmosphere
        int boxHeight = 50;
        ...
        // Create decorative border - use gold tones for victory
        Pixmap borderPixmap = new Pixmap(1, 1, Pixmap.Format.RGBA8888);
        borderPixmap.setColor(0.85f, 0.68f, 0.25f, 0.8f); // Gold
        borderPixmap.fill();
    ...
    private void setupUI() {
        ...
        // Middle empty space (to see the background image)
        root.add().expand().row();
        ...
        // If it's the last page, use gold color
        if (isLastLine()) {
    ...
    private void onContinueClicked() {
        currentLineIndex++;
        if (currentLineIndex >= dialogueData.lines.length) {
            // Dialogue ends, return to main menu
    ...
    public void render(float delta) {
        ...
        // Add a dimming overlay to make the dialog box clearer
