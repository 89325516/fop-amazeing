 ...
 * Core Features:
 * <ul>
 * <li>Automatic border wall generation (2-tile width)</li>
 * <li>Room + Corridor algorithm replacing perfect maze</li>
 * <li>Path connectivity verification after placing walls</li>
 * <li>Retry mechanism to ensure successful generation</li>
 * </ul>
 */
public class MapGenerator {
    ...
    // Grid status: 0 = Wall can be placed, 1 = Floor (Passage)
    private int[][] grid;

    // All wall entities
    private List<WallEntity> walls;

    // Safe zones (Start, Exit, Key surroundings)
    private Set<Long> safeZone;

    /**
     * Nested Map Configuration class.
     */
    ...
    private GenerationResult generate() {
        ...
        // 1. Initialization: Set all to floor
    ...
    /**
     * Randomly determines start and exit positions.
     * Ensures sufficient distance between them and keeps them within playable area.
     */
    private void determineStartAndExit(GenerationResult result) {
        ...
        for (int i = 0; i < maxAttempts; i++) {
            // Randomly generate two points (avoid buffer near borders)
    ...
    /**
     * Generates border walls (2-tile wide closed border).
     */
    ...
    /**
     * Generates internal maze (Room + Corridor algorithm).
     */
    ...
    private boolean canPlaceWall(int x, int y, int w, int h) {
        ...
        // Temporary mark (verification is skipped here for performance)
        for (int dx = 0; dx < w; dx++) {
            for (int dy = 0; dy < h; dy++) {
                grid[x + dx][y + dy] = 0;
            }
    ...
    private void addWall(int x, int y, int w, int h, boolean isBorder) {
        ...
        // Mark grid as occupied (Using full visual height 'h' to prevent overlapping
        // generation)
        for (int dx = 0; dx < w; dx++) {
            for (int dy = 0; dy < h; dy++) {
    ...
    /**
     * Marks a safe zone around a center point.
     */
    ...
    /**
     * Validates path connectivity using BFS.
     */
    ...
    /**
     * Finds a position for the key (furthest reachable point from start).
     */
    private Vector2 findKeyPosition(Vector2 start, Vector2 exit) {
        // Simplified: placing near center but biased towards exit
    ...
    /**
     * Generates a fallback map if generation fails.
     */
